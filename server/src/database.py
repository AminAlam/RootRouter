...
class database():
    ...
    def insert_image(self, file_path: str, file_size: float, file_format: str, img_size_x: int, img_size_y: int, img_size_z: int):
        ...
        cur.execute(sql, (file_path, file_size, file_format, img_size_x, img_size_y, img_size_z, unique_id))
        self.conn.commit()
        id = cur.lastrowid
        cur.close()
        return id
    ...
    def get_image_size(self, image_id: int):
        ...
        output = dict(zip(column_names, output))
        cur.close()
        img_size = (output['img_size_x'], output['img_size_y'], output['img_size_z'])
        return img_size
    ...
    def insert_cube(self, 
        ...
        cur = self.conn.cursor()
        cur.execute('BEGIN EXCLUSIVE')
        cur.execute(sql, (cube_x_start, cube_x_end, cube_y_start, cube_y_end, cube_z_start, cube_z_end, cube_size_bytes, image_id, cube_info_sent_to_server, cube_data_sent_to_server, cube_data))
        self.conn.commit()
        id = cur.lastrowid
        cur.close()
    ...
    def check_cube_exists(self, 
        ...
        cur = self.conn.cursor()
        cur.execute(sql, (cube_x_start, cube_x_end, cube_y_start, cube_y_end, cube_z_start, cube_z_end, image_id))
        output = cur.fetchone()
        cur.close()
        if output is not None:
            return output[0]
    ...
    def get_cubes_to_get_segmentation_results(self, segmentation_type: Union[str, str] = Union['soma', 'branch']):
        if segmentation_type not in ['soma', 'branch']:
            raise ValueError("Invalid segmentation type. Must be 'soma' or 'branch'.")
    ...
    def check_cube_sent_to_server(self, image_id: int, cube_x_start: int, cube_x_end: int, cube_y_start: int, cube_y_end: int, cube_z_start: int, cube_z_end: int):
        ...
        cur = self.conn.cursor()
        cur.execute(sql, (image_id, cube_x_start, cube_x_end, cube_y_start, cube_y_end, cube_z_start, cube_z_end))
        output = cur.fetchone()
        cur.close()
        if output is not None:
            return output[0]
    ...
    def insert_cell(self, image_id: int, soma_x_pos: int, soma_y_pos: int, soma_z_pos: int, cube_over_cell: bytes = None, segmented_cube_over_cell: bytes = None):
        cell_id = self.check_cell_exists(image_id, soma_x_pos, soma_y_pos, soma_z_pos)
        ...
        sql = ''' INSERT OR REPLACE INTO detected_cells(soma_x_pos, soma_y_pos, soma_z_pos, cube_over_cell, segmented_cube_over_cell, image_id)
                    VALUES(?,?,?,?,?,?) '''
        cur = self.conn.cursor()
        cur.execute(sql, (soma_x_pos, soma_y_pos, soma_z_pos, cube_over_cell, segmented_cube_over_cell, image_id))
        self.conn.commit()
    ...
    def get_cell(self, cell_id: int, image_id: int):
        cur = self.conn.cursor()
        cur.execute("SELECT * FROM detected_cells WHERE id = ? AND image_id = ?", (cell_id, image_id))
        row = cur.fetchone()
        if row is not None:
            column_names = [description[0] for description in cur.description]
    ...
    def get_cells_by_image_id(self, image_id: int):
        cur = self.conn.cursor()
        cur.execute("SELECT * FROM detected_cells WHERE image_id = ?", (image_id,))
        output = cur.fetchall()
    ...
    def update_cell(self, cell_id: int, image_id: int, soma_x_pos: int = None, soma_y_pos: int = None, soma_z_pos: int = None, cube_over_cell: bytes = None, segmented_cube_over_cell: bytes = None):
        ...
        cur = self.conn.cursor()
        cur.execute(sql, (soma_x_pos, soma_y_pos, soma_z_pos, cube_over_cell, segmented_cube_over_cell, cell_id, image_id))
        self.conn.commit()
        return cell_id
