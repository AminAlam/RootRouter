...
def test_plant_data_crud(client):
    ...
    with app.app_context():
        ...
        # Read
        retrieved = PlantData.query.filter_by(plant_name='CRUD Test Plant').first()
        ...
        db.session.commit()
        updated = PlantData.query.filter_by(plant_name='CRUD Test Plant').first()
        assert updated.moisture_value == 400
        
        # Delete
        db.session.delete(updated)
        db.session.commit()
        deleted = PlantData.query.filter_by(plant_name='CRUD Test Plant').first()
